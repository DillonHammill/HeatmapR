...
* **HeatmapR** is built using base graphics, so there is no required `ggplot2` or `plotly` knowledge in order to use this package.
...
## Acknowledgements
**HeatmapR** relies on statistical methods in the `stats` package to compute distance matrices and perform hiearchical clustering. **HeatmapR** also uses some modified `stats` code from the  [ggdendro](https://github.com/andrie/ggdendro) package to get the co-ordinates for the dendrogram line segments. 
